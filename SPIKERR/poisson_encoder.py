...
class PoissonEncoder(t.nn.Module):
    def __init__(self, spike_train_count: Optional[int]):
        super(PoissonEncoder, self).__init__()

        if spike_train_count < 0:
            raise ValueError("spike_train_count must be a positive integer or None.")

        self.spike_train_count = spike_train_count

    def forward(self, x: t.Tensor) -> Generator[Tuple[Spikes, Index], None, None]:
        return poisson_encode(x, self.spike_train_count)


def poisson_encode(
    data: t.Tensor, spike_train_count: Optional[int]
) -> Generator[Tuple[Spikes, Index], None, None]:
    """
